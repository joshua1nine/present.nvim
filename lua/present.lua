...
local execute_lua_code = function(block)
	...
	local chuck = loadstring(block.body)
	pcall(function()
		table.insert(output, "")
	...
	-- Restore the original print fucntion
...
local parse_slides = function(lines)
	...
	for _, line in ipairs(lines) do
		-- "#" at the beginning of a line indicates a slide
...
local create_window_configurations = function()
	...
	local footer_height = 1 -- 1, no boder
...
M.start_presentation = function(opts)
	-- Init opts and grab the buffer with
	opts = opts or {}
	...
	-- Create a list of the users options we want to change
	-- then restore
	local restore = {
		cmdheight = {
	...
	vim.api.nvim_create_autocmd("VimResized", {
		...
		callback = function()
			...
			-- Re-calculates current slide size
			set_slide_content(state.current_slide)
		end,
	})
